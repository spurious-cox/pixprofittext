...
for f in dist/PixProFitText.app/Contents/Frameworks/*.dylib; do
    [[ -f "$f" ]] || continue
...
find dist/PixProFitText.app -type f -print0 2>/dev/null | while IFS= read -r -d $'\0' f; do
...
find dist/PixProFitText.app -name '*.framework' -print0 2>/dev/null \
    | xargs -0 -n1 -I{} codesign --force --timestamp --options runtime --sign "$SIGN_ID" {} 2>/dev/null || true
codesign --force --timestamp --options runtime \
    --entitlements pixprofittext.entitlements --sign "$SIGN_ID" dist/PixProFitText.app
codesign --verify --deep --strict dist/PixProFitText.app
...
if [[ "$1" != "--no-install" ]]; then
    ...
    cp -R dist/PixProFitText.app /Applications/
    xattr -dr com.apple.quarantine /Applications/PixProFitText.app 2>/dev/null || true
    codesign -dv /Applications/PixProFitText.app 2>&1 | grep -E "Identifier=|Authority="
    plutil -extract CFBundleShortVersionString raw /Applications/PixProFitText.app/Contents/Info.plist
fi

echo "==> running instances: $(pgrep -x PixProFitText | wc -l | tr -d ' ')"
